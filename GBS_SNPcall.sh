...
awk '$10 <= 1 && $4 !~/,/ { print $1 "_" $2 " " $3 " " $4 " " ($8+($9*2))/28}' pop07H.varscan > GBS_p07H_10miss.freq # 10 % missing 1.4

awk '$10 <= 2 && $4 !~/,/ { print $1 "_" $2 " " $3 " " $4 " " ($8+($9*2))/28}' pop07H.varscan > GBS_p07H_20miss.freq # 20 % missing 2.8
...
awk '$10 <= 2 && $4 !~/,/ { print $1 "_" $2 " " $3 " " $4 " " ($8+($9*2))/50}' pop11U.varscan > GBS_p11U_10miss.freq # 10 % missing 2.5

awk '$10 <= 5 && $4 !~/,/ { print $1 "_" $2 " " $3 " " $4 " " ($8+($9*2))/50}' pop11U.varscan > GBS_p11U_20miss.freq # 20 % missing 5
